...
class MSPiperMotorsBus:
    ...
    def __init__(self,
        ...
        self.slaver_motors = config.slaver_motors
        self.master_motors = config.master_motors
    ...
    def motor_names(self) -> list[str]:
        return list(self.slaver_motors.keys())

    @property
    def motor_models(self) -> list[str]:
        return [model for _, model in self.slaver_motors.values()]

    @property
    def motor_indices(self) -> list[int]:
        return [idx for idx, _ in self.slaver_motors.values()]


    @property
    def M_motor_names(self) -> list[str]:
        return list(self.master_motors.keys())

    @property
    def M_motor_models(self) -> list[str]:
        return [model for _, model in self.master_motors.values()]

    @property
    def M_motor_indices(self) -> list[int]:
        return [idx for idx, _ in self.master_motors.values()]
    ...
    def read(self) -> Dict:
        ...
        return {
            ...
            "gripper": gripper_state.grippers_angle / 1000000
        }
    def read_M(self) ->Dict:
        """
           读取主臂关节位置
           - 机械臂关节消息,单位0.001度
           - 机械臂夹爪消息
        ...
        return {
            ...
            "gripper": gripper_ctrl.grippers_angle / 1000000
        }
