    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        ...
        for key, q_idx in query_indices.items():
            if key in self.meta.video_keys:
                continue
            # 从数据集中选择数据，返回的是一个列表
            selected_items = self.hf_dataset.select(q_idx)[key]
            # 将列表中的每个元素转换为张量，然后堆叠成一个批处理张量
            output[key] = torch.stack([torch.tensor(item) for item in selected_items])
        return output

        # return {
        #     key: torch.stack(self.hf_dataset.select(q_idx)[key]) for key, q_idx in query_indices.items()
